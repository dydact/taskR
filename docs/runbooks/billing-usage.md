...
```
make billing-console ARGS="--base-url http://localhost:8000 --tenant <tenant> plan"
```
Expect JSON containing `plan_slug`, `status`, and the resolved `features` set.

## 2. Flip plan tiers
```
make billing-console ARGS="--base-url http://localhost:8000 --tenant <tenant> set-plan --plan starter"
make billing-console ARGS="--base-url http://localhost:8000 --tenant <tenant> set-plan --plan growth"
```
...
```
make billing-console ARGS="--base-url http://localhost:8000 --tenant <tenant> set-override --feature flows.core"
make billing-console ARGS="--base-url http://localhost:8000 --tenant <tenant> overrides"
make billing-console ARGS="--base-url http://localhost:8000 --tenant <tenant> delete-override --feature flows.core"
```
...
- With `starter` plan (no override), call the flows run endpoint:
  - `curl -H "x-tenant-id: <tenant>" http://localhost:8000/flows/templates/foo/run` → expect `403`.
- Re-enable the `flows.core` override and retry → expect `201`.

## 5. Download usage report
```
make billing-export ARGS="--taskr-url http://localhost:8000 --tenant <tenant> --days 7 --output usage.csv"
```
